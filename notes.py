...
def load():
    try:
        ...
        print("Ошибка !Файл поврежден/неверный формат.")
        return {}
...
def showAll(notes):
    if(notes):
        ...
        note_id = input("Для открытия заметки введите ID. Для возвращения назад введите  \"no\": ")
        if note_id.lower() != "no":
...
def add(notes, id_counter):
    titleNote = input("Введите заголовок: ")
